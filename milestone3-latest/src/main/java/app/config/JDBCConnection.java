...
public class JDBCConnection {
    ...
    public List<Map<String, Object>> getSimilarGroups(String commodityCode,
        ...
        try {
            ...
            if (similarityMetric.equals("avg")) {
                ...
                sql += "         select *,abs(val-(select val from t where groupp=substr('" + commodityCode
                        + "',1,3))) as diff";
                sql += "         from t";
                ...
                sql += "    ),res as ( ";
                sql += "    select ";
                sql += "        *,abs(val-(select val from t where groupp=substr('" + commodityCode
                        + "',1,3))) as diff ";
                sql += "    from ";
                sql += "        t ";
                sql += "    where ";
    ...
    public List<FoodLossEvent> getFoodLossEvents(List<String> country, String startYear, String endYear,
        ...
        try {
            ...
            StringBuilder lossQuery = new StringBuilder(
                    "SELECT t1.country, t1.start_loss, t1.activity, t1.commodity, t1.cause_of_loss, t1.food_supply_stage, t2.end_loss, t2.activity, t2.commodity, t2.cause_of_loss, t2.food_supply_stage ");
            lossQuery.append(
                    "FROM (SELECT country, avg(loss_percentage) as start_loss, activity, commodity, cause_of_loss, food_supply_stage FROM FoodLossEvent WHERE country IN (");
            ...
            lossQuery.append(
                    "JOIN (SELECT country, avg(loss_percentage) as end_loss, activity, commodity, cause_of_loss, food_supply_stage FROM FoodLossEvent WHERE country IN (");
            ...
            while (resultSet.next()) {
                ...
                if (isEmptyString(activity))
                    activity = null;
                if (isEmptyString(commodity))
                    commodity = null;
                if (isEmptyString(causeOfLoss))
                    causeOfLoss = null;
                if (isEmptyString(supplyStage))
                    supplyStage = null;
                ...
                if (startLoss != null && endLoss != null) {
                    ...
                    FoodLossEvent event = new FoodLossEvent(Arrays.asList(countryName),
                            String.format("%.3f%%", dStartLoss), String.format("%.3f%%", dEndLoss),
                            increaseOrDecrease + formattedLossDiff, startYear, endYear, commodity, activity,
                            supplyStage, causeOfLoss);
                    events.add(event);
                } else {
                    FoodLossEvent event = new FoodLossEvent(Arrays.asList(countryName),
                            String.format("%.3f%%", dStartLoss), String.format("%.3f%%", dEndLoss), "N/A", startYear,
                            endYear, commodity, activity, supplyStage, causeOfLoss);
                    events.add(event);
                }
            }
        } catch (SQLException e) {
    ...
    public List<SimilarityScore> getSimilarityScores(String location, String year, String similarityType,
            String similarityLevel, int numResults) {
    ...
    public List<SimilarityScore> calculateSimilarityScores(List<SimilarityData> dataList, String calculationMethod,
            String criteria) {
